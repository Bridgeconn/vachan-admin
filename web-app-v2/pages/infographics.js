...
export const getStaticProps = async () => {
  ...
  const languageArray = bibles.map((item) => {
    return {
      name: item.languageVersions[0].language.name,
      code: item.languageVersions[0].language.code,
    };
  });

  const promises = languageArray.map((language) => {
    return fetch(
      "https://api.autographamt.com/v1/infographics/" + language.code
    ).then((data) => data.json());
  });

  const infographicsArray = await Promise.all(promises);
  return {
    props: {
      infographicsArray,
    },
...
const showInfographicsData = (infographicsArray) => {
  const result = [];
  const showData = infographicsArray.forEach((item) => {
    console.log(item);
    if (item.success !== false) {
      item.books.map((val) => {
        val.infographics.map((item2) => {
          let infographic = {
...
function Infographics({ infographicsArray }) {
  ...
  const data = React.useMemo(
    () => showInfographicsData(infographicsArray),
    [infographicsArray]
  );
